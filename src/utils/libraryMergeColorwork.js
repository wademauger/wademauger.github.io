...
export function mergeColorworkIntoLibrary(library, patternPayload) {
  ...
  const idxById = patternPayload && patternPayload.id ? lib.colorworkPatterns.findIndex(p => p.id === patternPayload.id) : -1;
  const idxByName = patternPayload && patternPayload.name ? lib.colorworkPatterns.findIndex(p => p.name === patternPayload.name) : -1;
